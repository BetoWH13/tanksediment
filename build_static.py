...
<head>
  ...
  <style>
    *, *::before, *::after {{ box-sizing: border-box; margin: 0; padding: 0; }}
    ...
    .hub-lead {{ background:linear-gradient(135deg,#f5f9fb,#e8f1f5); border:1px solid #d7e5ec; border-radius:12px; padding:1.6rem; }}
    .hub-lead p {{ color:#4b5c65; margin:0; max-width:680px; }}
    ...
    .featured-grid {{ display:grid; grid-template-columns:repeat(3,1fr); gap:1rem; margin:1rem 0 2.4rem; }}
    .featured-card {{ border:1px solid #dce6eb; border-radius:12px; padding:1.35rem; background:#fff; box-shadow:0 6px 20px rgba(20,50,65,.06); display:flex; flex-direction:column; min-height:235px; }}
    ...
    .article-grid {{ display:grid; grid-template-columns:repeat(3,1fr); gap:.9rem; margin-top:1rem; }}
    .article-card {{ border:1px solid #e0e7ea; border-radius:10px; padding:1.15rem; background:#fff; display:flex; flex-direction:column; min-height:210px; transition:transform .15s ease, box-shadow .15s ease; }} .article-card:hover {{ transform:translateY(-2px); box-shadow:0 8px 20px rgba(20,50,65,.08); }} .article-card h2 {{ font-size:1.05rem; margin:0 0 .45rem; line-height:1.35; }} .article-card p {{ color:#617078; font-size:.88rem; flex:1; margin-bottom:.8rem; }}
...
for post in posts:
    ...
    write_page(f"{slug}/index.html", render_page(post["post_title"], body, slug))
    written += 1
